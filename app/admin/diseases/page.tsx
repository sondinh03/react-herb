...
import { DiseaseForm, DiseasesResponse } from "@/app/types/diseases";
import { HerbResponse, HerbResponse } from "@/types/api";
...
export default function DiseasesPage() {
  ...
  const [editingId, setEditingId] = useState<number | null>(null); // ID của công dụng đang sửa
  const [form, setForm] = useState<DiseaseForm>({ name: "", description: "" }); // Dữ liệu form
  ...
  const columns = [
    {
      ...
      cell: (disease: Disease) => (
        <span className="font-medium">{disease.id}</span>
      ),
      className: "w-[50px]",
    },
    {
    ...
    },
    {
      key: "description",
      header: "Mô tả",
      cell: (disease: Disease) => disease.description || "-",
    },
    {
      key: "actions",
      header: "Thao tác",
  ...
  const handleEditDisease = async (id: number) => {
    const disease = await fetchApi<HerbResponse<DiseasesResponse>>(`/api/diseases/${id}`, {
      method: "GET",
    });
  ...
  const handleViewDisease = (diseaseId: number) => {
    router.push(`/admin/diseases/${diseaseId}`);
  };

  // Xử lý xóa công dụng
  const handleDeleteDisease = async (id: number) => {
    try {
      await fetchApi<null>(`/api/diseases/${id}`, {
        method: "DELETE",
      });
      toast({
        title: "Thành công",
        description: "Đã xóa công dụng chữa bệnh",
        variant: "success",
      });
      ...
      toast({
        title: "Lỗi",
        description: error.message || "Không thể xóa công dụng",
        variant: "destructive",
      });
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        // API: Cập nhật công dụng
        const id = editingId;
        await fetchApi<HerbResponse<Boolean>>(`/api/diseases/edit/${id}`, {
          method: "PUT",
        ...
        toast({
          title: "Thành công",
          description: "Đã cập nhật công dụng chữa bệnh",
          variant: "success",
        });
      } else {
        // API: Thêm công dụng mới
        await fetchApi<HerbResponse<DiseasesResponse>>(`/api/diseases/create`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify(form),
        });
        toast({
          title: "Thành công",
          description: "Đã thêm công dụng chữa bệnh",
          variant: "success",
        });
      }
      ...
      toast({
        title: "Lỗi",
        description: error.message || "Không thể lưu công dụng",
        variant: "destructive",
      });
    } finally {
  ...
  const handleRowClick = (disease: Disease) => {
  ...
  return (
    <div className="py-6 px-4 sm:px-6 lg:px-8">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-gray-900">
            Quản lý công dụng chữa bệnh
          </h1>
          <p className="mt-1 text-sm text-gray-500">
            Quản lý thông tin các công dụng chữa bệnh trong hệ thống
          </p>
        </div>
      ...
      {isAdding && (
        <div className="mb-6 p-6 bg-white rounded-lg shadow">
          <h2 className="text-lg font-semibold mb-4">
            {editingId ? "Sửa công dụng" : "Thêm công dụng mới"}
          </h2>
      ...
      <SearchPanel
        ...
        showExport={true}
        placeholder="Tìm kiếm công dụng chữa bệnh..."
      />
      ...
      <div className="mt-6">
        <DataTable
          ...
          emptyMessage="Không tìm thấy công dụng chữa bệnh nào"
          labels={{
            previous: "Trước",
            next: "Sau",
